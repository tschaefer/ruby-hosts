...
RSpec.describe Hosts::File do
  ...
  describe '#parse_file' do
    context 'when file is not existing' do
      it 'raises an error' do
        expect { hostsfile.send(:parse_file, '/no/such/file') }.to raise_error(/No such file/)
      end
    end
    ...
    context 'when file is existing and valid' do
      ...
      it 'returns an array with elements', :aggregate_failures do
        entries = hostsfile.send(:parse_file, assert)
  ...
  describe '#write_file' do
    context 'when file is not existing' do
      it 'raises an error' do
        expect { hostsfile.send(:write_file, '/no/such/file', '') }.to raise_error(/No such file/)
      end
    end
    ...
    context 'when file is existing and has no entries' do
      ...
      it 'writes a legend line' do
        hostsfile.send(:write_file, tempfile, '')
